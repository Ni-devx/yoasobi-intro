...
--   返値:
--     status        — ok / wrong / timeout / invalid_attempt
...
begin
  ...
  if not found then
    return query select 'invalid_attempt'::text,null::integer,null::text,null::integer,null::integer,null::uuid;
    return;
  end if;

  v_elapsed_ms := greatest(1,
  ...
  if v_elapsed_ms > 10000 then
    ...
    return query select 'timeout'::text,null::integer,null::text,null::integer,null::integer,null::uuid;
    return;
  end if;

  -- 回答検証
  select (p_answer_norm = any(answers_normalized)) into v_correct
  from public.songs where id = v_attempt.song_id;
  ...
  if not v_correct then
    ...
    delete from public.attempts where id = p_attempt_id;
    return query select 'wrong'::text,null::integer,null::text,null::integer,null::integer,null::uuid;
    return;
  end if;
  ...
  if v_attempt.scope = 'single' then
    select time_ms into v_30th_ms
    from public.scores
    where scope = 'single' and mode = v_attempt.mode
      and song_id = v_attempt.song_id and confirmed = true
    order by time_ms asc limit 1 offset 29;

    if v_30th_ms is null or v_elapsed_ms < v_30th_ms then
      insert into public.scores (scope, mode, song_id, time_ms)
      values ('single', v_attempt.mode, v_attempt.song_id, v_elapsed_ms)
      returning id into v_score_id;
    end if;

    return query select 'ok'::text, v_elapsed_ms, null::text, null::integer, null::integer, v_score_id;
    return;
  end if;
  ...
  if v_next_pos > v_run.total_songs then
    -- 全曲完走
    v_total_ms := v_run.total_ms_so_far + v_elapsed_ms;
    update public.marathon_runs
    set status = 'completed', current_pos = v_next_pos, total_ms_so_far = v_total_ms
    where id = v_run.id;

    select time_ms into v_30th_ms
    from public.scores
    where scope = 'marathon' and mode = v_run.mode
      and song_id is null and confirmed = true
    order by time_ms asc limit 1 offset 29;
    ...
    return query select 'completed'::text, v_elapsed_ms, null::text, null::integer, v_total_ms, v_score_id;
    return;
  end if;
  ...
  set current_pos     = v_next_pos,
      total_ms_so_far = total_ms_so_far + v_elapsed_ms
  where id = v_run.id;

  return query
    select 'next'::text, v_elapsed_ms,
